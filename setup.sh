...
# 流程:
#   阶段0: Mihomo 代理服务 (可选)
#   阶段1: 配置生成 (系统名/IP/密码 → uniweb-system.config)
#   阶段2: 配置应用 (替换 initHome/initData 中的 #{KEY} 占位符)
#   阶段3: 选择安装组件 (whiptail 多选菜单)
#   阶段4: 按需分发文件 (initHome/initData/script → 目标目录)
#   阶段5: 执行安装 (Docker + Registry → 拉取镜像 → 基础服务 → 微服务 → 前端 → SaaS → 开发服务)
# 配置文件:
...
# ============================================================
#  系统依赖（直接执行 setup.sh 时使用）
# ============================================================

install_system_deps() {
    local _missing=false
    for cmd in git curl bash whiptail gpg htpasswd locale-gen update-ca-certificates; do
...
generate_config() {
    ...
    HOST_IP="127.0.0.1"
    local _manual_opt="手动输入IP或域名"
    local _ip_count
    _ip_count=$(echo "$HOST_IPS" | wc -w)

    case "$_ip_count" in
        0) echo "未检测到本机IP地址，请选择手动输入。" ;;
        1) echo "本机检测到IP: $HOST_IPS" ;;
        *) echo "本机检测到多个IP，请选择一个IP作为配置文件的主机配置:" ;;
    esac
    PS3="请输入编号 [1-$((_ip_count + 1))]: "
    select HOST_IP in $HOST_IPS "$_manual_opt"; do
        if [ -n "$HOST_IP" ]; then
            if [ "$HOST_IP" = "$_manual_opt" ]; then
                while true; do
                    read -e -p "请输入IP或域名: " HOST_IP
                    if [ -n "$HOST_IP" ]; then
                        log_info "您输入的地址是: $HOST_IP"
                        break 2
                    else
                        echo "输入不能为空，请重新输入。"
                    fi
...
apply_config() {
    ...
    while IFS='=' read -r key value; do
        ...
        find "$REPO_DIR" -type f -not -path "*/.git/*" -print0 | xargs -0 sed -i "s\x01#{${key}}\x01${value}\x01g"
        if [[ $key == *"_PASSWORD" ]]; then
            local bcrypt_value
            bcrypt_value=$(htpasswd -nbBC 10 test "$value" | cut -c 6-)
            log_info "替换BCRYPT密码变量 ${key}_BCRYPT"
            find "$REPO_DIR" -type f -not -path "*/.git/*" -print0 | xargs -0 sed -i "s\x01#{${key}_BCRYPT}\x01${bcrypt_value}\x01g"
        fi
    done < "$CONFIG_FILE"
...
${codename} stable" | tee /etc/apt/sources.list.d/docker.list > /dev/null
    ...
    log_info "检查 sshd 是否因 OpenSSL 版本变化而异常..."
    if ! sshd -t 2>/dev/null; then
        log_warn "sshd 配置检测失败，可能 OpenSSL 版本不匹配，正在重装..."
        apt-get install -y --reinstall openssh-server openssh-client || true
        systemctl restart sshd 2>/dev/null || systemctl restart ssh 2>/dev/null || true
        if sshd -t 2>/dev/null; then
            log_ok "sshd 修复成功"
        else
            log_error "sshd 修复失败，请手动检查"
        fi
    fi
...
_docker_pull() {
    ...
    for attempt in 1 2 3 4 5; do
        local pull_output
        pull_output=$(timeout 300 docker pull "$ref" 2>&1) || true
...
pull_selected_images() {
    ...
    # uw-gateway (索引0) 由末尾无条件启动，此处从索引1开始
    images+=("${IMAGE_UW_GATEWAY}")
...
EOF
    ...
    sysctl -p /etc/sysctl.conf 2>/dev/null | tail -5 >> "$LOG_FILE"
...
' -X PUT "${LOG_ES_SERVER}/_index_template/uw-biz-log")
    ...
    find "/home/kibana5601/config" -type f -exec sed -i "s\x01#{ELASTICSEARCH_SERVICE_ACCOUNT_TOKEN}\x01${ELASTICSEARCH_SERVICE_ACCOUNT_TOKEN}\x01g" {} +
...
EOF
    ...
    log_ok "从机安装命令:"
    echo ""
    echo -e "  ${CYAN}curl -fsSL http://${SYSTEM_SERVER}:${port}/slaveInstaller.sh | bash${NC}"
    echo ""
    log_info "管理命令:"
...
show_config
# 编辑uniweb-system.config配置文件
read -e -p "是否需要编辑uniweb-system.config配置文件? [y/N]: " EDIT_IT
case "$EDIT_IT" in
...
# --- 阶段 2: 配置应用 (替换 #{KEY} 占位符) ---
log_step "===== 阶段 2: 配置应用 ====="
apply_config

# --- 阶段 3: 选择安装组件 (whiptail 多选菜单) ---
log_step "===== 阶段 3: 选择安装组件 ====="
...
# --- 阶段 4: 按需分发文件 (仅复制选中组件对应的 initHome/initData) ---
log_step "===== 阶段 4: 按需分发文件 ====="
...
read -e -p "是否需要编辑uniweb-registry.config配置文件? [y/N]: " EDIT_IT
case "$EDIT_IT" in
    y|Y) ${EDITOR:-vi} "$REGISTRY_FILE" ;;
    *) ;;
esac
...
# uw-gateway 为必选组件，最后启动为了可以最终访问到所有服务
sleep 10 
